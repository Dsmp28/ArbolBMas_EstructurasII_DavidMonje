package org.example;

import java.util.List;

public class Main {
    public static void main(String[] args) {
        ArbolBMas arbol = new ArbolBMas(5);
        //arbol.procesarCsv("/Users/david/IdeaProjects/ArbolBMas_EstructurasII_DavidMonje/src/main/resources/datos_10000.csv");
        //arbol.mostrarArbol();
        arbol.insertar("01", new Libro("1", "Libro 1", "Autor 1", "Categoria 1", 100.0, 10));
        arbol.insertar("02", new Libro("2", "Libro 2", "Autor 2", "Categoria 2", 200.0, 20));
        arbol.insertar("03", new Libro("3", "Libro 3", "Autor 3", "Categoria 3", 300.0, 30));
        arbol.insertar("04", new Libro("4", "Libro 4", "Autor 4", "Categoria 4", 400.0, 40));
        arbol.insertar("05", new Libro("5", "Libro 5", "Autor 5", "Categoria 5", 500.0, 50));
        arbol.insertar("06", new Libro("6", "Libro 6", "Autor 6", "Categoria 6", 600.0, 60));
        arbol.insertar("07", new Libro("7", "Libro 7", "Autor 7", "Categoria 7", 700.0, 70));
        arbol.insertar("08", new Libro("8", "Libro 8", "Autor 8", "Categoria 8", 800.0, 80));
        arbol.insertar("09", new Libro("9", "Libro 9", "Autor 9", "Categoria 9", 900.0, 90));
        arbol.insertar("10", new Libro("10", "Libro 10", "Autor 10", "Categoria 10", 1000.0, 100));
        arbol.insertar("11", new Libro("11", "Libro 11", "Autor 11", "Categoria 11", 1100.0, 110));
        arbol.insertar("12", new Libro("12", "Libro 12", "Autor 12", "Categoria 12", 1200.0, 120));
        arbol.insertar("13", new Libro("13", "Libro 13", "Autor 13", "Categoria 13", 1300.0, 130));
        arbol.insertar("14", new Libro("14", "Libro 14", "Autor 14", "Categoria 14", 1400.0, 140));
        arbol.insertar("15", new Libro("15", "Libro 15", "Autor 15", "Categoria 15", 1500.0, 150));
        arbol.insertar("16", new Libro("16", "Libro 16", "Autor 16", "Categoria 16", 1600.0, 160));
        arbol.insertar("17", new Libro("17", "Libro 17", "Autor 17", "Categoria 17", 1700.0, 170));
        arbol.insertar("18", new Libro("18", "Libro 18", "Autor 18", "Categoria 18", 1800.0, 180));
        arbol.insertar("19", new Libro("19", "Libro 19", "Autor 19", "Categoria 19", 1900.0, 190));
        arbol.insertar("20", new Libro("20", "Libro 20", "Autor 20", "Categoria 20", 2000.0, 200));
        arbol.insertar("21", new Libro("21", "Libro 21", "Autor 21", "Categoria 21", 2100.0, 210));
        arbol.insertar("22", new Libro("22", "Libro 22", "Autor 22", "Categoria 22", 2200.0, 220));
        arbol.eliminar("07");
        arbol.eliminar("13");

        arbol.mostrarArbol();
    }
}